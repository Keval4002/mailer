        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-[360px_1fr] gap-6 items-start">
          ...
          <section className="bg-surface border border-border rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="px-5 min-h-[60px]">
              {contacts.length === 0 ? (
                <div className="text-text-muted text-sm text-center py-8">No contacts yet. Click &quot;+ Add&quot; to begin.</div>
              ) : (
                contacts.map((c, i) => (
                  <div key={i} className="group relative grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-2 py-4 border-b border-border last:border-0 transition-colors hover:bg-neutral-soft hover:-mx-5 hover:px-5 hover:rounded-lg">
                    <div className="col-span-1 sm:col-span-2 text-[10px] font-semibold uppercase tracking-wider text-text-muted mb-1">Contact {i + 1}</div>
                    <input type="email" placeholder="Email *" required className="col-span-1 sm:col-span-2" value={c.email} onChange={(e) => updateContact(i, "email", e.target.value)} />
                    ...
                    <input type="text" placeholder="Title" className="col-span-1 sm:col-span-2" value={c.title} onChange={(e) => updateContact(i, "title", e.target.value)} />
                    <button type="button" onClick={() => removeContact(i)} className="absolute top-3 right-0 opacity-0 group-hover:opacity-100 group-hover:right-3 p-1 rounded bg-transparent text-text-muted hover:bg-danger-soft hover:text-danger transition-all">
                      <X size={16} />
                    </button>
                  </div>
                ))
          ...
          <section className="bg-surface border border-border rounded-xl shadow-sm overflow-hidden">
            <div className="flex items-start justify-between gap-3 border-b border-border p-5 pb-4">
              <div>
                ...
                <p className="text-text-muted text-sm mt-1">
                  Edit the body for each step. <code>{"{name}"}</code> and <code>{"{company}"}</code> will be personalized per contact.
                </p>
              </div>
            ...
            <div className="p-5 px-6 flex flex-col gap-0">
              ...
              <div className="relative grid grid-cols-[24px_1fr] gap-5 after:content-[''] after:absolute after:left-[11px] after:top-[28px] after:bottom-[-8px] after:w-[2px] after:bg-border">
                <div className="relative z-10 w-6 h-6 rounded-full border-[3px] border-accent bg-surface shadow-[0_0_0_4px_rgba(0,0,0,0.05)] mt-3.5" />
              ...
              {includeFollowups && (
                <div className="relative grid grid-cols-[24px_1fr] gap-5 after:content-[''] after:absolute after:left-[11px] after:top-[28px] after:bottom-[-8px] after:w-[2px] after:bg-border">
                  <div className="relative z-10 w-6 h-6 rounded-full border-[3px] border-info bg-surface shadow-[0_0_0_4px_rgba(0,145,255,0.1)] mt-3.5" />
                  <div className="min-w-0 bg-surface border border-border rounded-lg p-5 mb-6 shadow-sm hover:shadow-md hover:border-border-strong transition-all">
                    <div className="flex items-center flex-wrap gap-3 mb-4">
              ...
              {includeFollowups && (
                <div className="relative grid grid-cols-[24px_1fr] gap-5">
                  <div className="relative z-10 w-6 h-6 rounded-full border-[3px] border-border-strong bg-surface mt-3.5" />
                  <div className="min-w-0 bg-surface border border-border rounded-lg p-5 mb-6 shadow-sm hover:shadow-md hover:border-border-strong transition-all">
                    <div className="flex items-center flex-wrap gap-3 mb-4">
